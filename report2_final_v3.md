...
K-means clustering (k=4) was applied to each participant's SpAM coordinates to derive cluster labels. To validate this choice empirically, the elbow method (mean inertia) and silhouette scoring were computed across k=2 to k=8 on all 105 participant-domain SpAM subsets (Table H1-Val). The silhouette score peaks at k=2 (0.5017) and declines monotonically; k=4 (silhouette=0.4213) retains a respectable score and represents the largest k still meaningfully above the k=5 drop-off (0.3552). Crucially, the inertia elbow also occurs between k=3 and k=5, with k=4 sitting at the bend. The choice of k=4 is therefore both theoretically grounded in the four expected semantic groupings per domain and consistent with the data-driven validation. Each consecutive VFT word pair was then labelled within-cluster or between-cluster based on shared cluster membership.

**Table H1-Val: Elbow and Silhouette metrics across k=2 to 8**

| k | Mean Inertia | Mean Silhouette | N subsets |
|---|---|---|---|
| 2 | 0.3127 | **0.5017** | 104 |
| 3 | 0.1508 | 0.4638 | 104 |
| **4** | **0.0819** | **0.4213** | 99 |
...
*Row k=4 in bold — the theoretically and empirically supported choice. Silhouette peaks at k=2 but that would merge all domain-level structure into just two coarse groups; k=4 is the smallest k that captures the expected intra-domain groupings while remaining well above the k≥5 performance drop.*

![Figure H1-Val: Elbow and Silhouette](k_selection_plots.png)
*Figure (H1 Validation): Left — Mean inertia across k=2–8 showing the elbow at k=3–4. Right — Average silhouette score, peaking at k=2 with a secondary plateau at k=4 before a steeper drop at k=5 (dashed red line at k=2 = automatic peak; k=4 is the theoretically preferred choice).*
...
4. **Clustering validation completed, k=4 is a justified but not optimal choice.** Elbow and silhouette analyses confirmed k=4 is consistent with the data (silhouette=0.421, inertia elbow at k=3–4). However, the silhouette score peaks at k=2 (0.502), suggesting participants' SpAM arrangements may form only two broad groups rather than four. Future work should examine whether a two-cluster solution (e.g., typical vs atypical exemplars, or animate vs inanimate) might be more appropriate for some domains.
