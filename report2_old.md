...
K-means clustering (k=4) was applied to each participant's SpAM coordinates to derive cluster labels. The choice of k=4 was theoretically motivated by the four broad semantic groupings expected within each domain. Each consecutive VFT word pair was labelled within-cluster or between-cluster based on shared cluster membership.
...
**Visualisation rationale:** A horizontal coefficient plot for both models side by side allows direct visual comparison. Bar direction (left/right) shows whether each predictor speeds up or slows retrieval; bar length shows magnitude; error bars show ±1 SE; colour indicates significance.
...
4. **Clustering method not validated empirically.** K-means was applied with k=4, theoretically motivated by the four semantic domains. Future work should use the elbow method or silhouette scoring to determine the empirically optimal number of clusters per participant and domain.
